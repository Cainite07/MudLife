...
import com.hualala.linyu.api.getUserProjectsSafe
...
class MainViewModel : ViewModel() {
    ...
    // 常用饮水机列表
    var recentDevices by mutableStateOf<List<com.hualala.linyu.model.RecentDevice>>(emptyList())
    ...
    var qzhqUseCodeData by mutableStateOf<com.hualala.linyu.model.QzhqUseCodeData?>(
        if (PrefsHelper.qzhqRandomCode.isNotEmpty()) {
            com.hualala.linyu.model.QzhqUseCodeData(
        ...
        viewModelScope.launch {
            try {
                ...
                if (resp.success && resp.data != null) {
                    ...
                    val recentDev = com.hualala.linyu.model.RecentDevice(
    ...
    fun startRecentDevice(dev: com.hualala.linyu.model.RecentDevice, phone: String) {
        ...
        viewModelScope.launch {
            try {
                ...
                if (devPid != null && NetworkModule.getAccountIdForProject(devPid) == null) {
                    try {
                        ...
                        if (pResp.success && !pResp.data.isNullOrEmpty()) {
                            ...
                            com.hualala.linyu.utils.AppLogger.i("Shower", "开阀前补全多项目账户映射: $map")
                        }
                    } catch (_: Exception) {}
                }
                ...
                if (!resp.success) {
                    ...
                    com.hualala.linyu.utils.AppLogger.e("Shower", "出水失败: $msg | 设备: ${device.displayName}, sn: $snCode, projectId: $devPid, accountId: ${devAuth["accountId"]}")
                ...
                if (!opened) {
                    ...
                    com.hualala.linyu.utils.AppLogger.e("Shower", "开阀确认失败: 9次轮询未返回开启状态 | 设备: ${device.displayName}, sn: $snCode, devPid: $devPid")
                    mqttManager?.disconnect()
                    return@launch
                }
                ...
                PrefsHelper.addRecentDevice(com.hualala.linyu.model.RecentDevice(
            ...
            try {
                ...
                if (!close.success) {
                    ...
                    if (close.errorCode == 307 || close.displayMessage?.contains("已在") == true) {
                        ...
                        com.hualala.linyu.utils.AppLogger.e("Shower", "关阀请求失败: ${close.displayMessage} | sn: $snCode, 订单: $orderNo, devPid: $devPid")
                        isStopping = false
                    }
                    return@launch
                }
    ...
    private fun checkKick(msg: String?) {
        ...
        if (isRealKick) {
            com.hualala.linyu.utils.AppLogger.e("Auth", "触发账号强制下线: $msg")
            kickedOut = true
            PrefsHelper.clear()
        }
    }
    private fun checkKickEx(e: Exception) {
        ...
        if (m.contains("401") || m.contains("403") || m.contains("Unauthorized") || m.contains("Forbidden")) {
            com.hualala.linyu.utils.AppLogger.e("Auth", "触发网络鉴权失效下线: $m")
            kickedOut = true
            PrefsHelper.clear()
        }
    }
    ...
    fun loadQzhqUseCode() {
        ...
        viewModelScope.launch {
            qzhqLoading = true
            try {
                val resp = com.hualala.linyu.api.QzhqNetwork.getUseCode(phone, loginCode)
    ...
    fun toggleQzhqStatus(newStatus: Int) {
        ...
        viewModelScope.launch {
            try {
                val resp = com.hualala.linyu.api.QzhqNetwork.updateUseCodeStatus(newStatus, phone, loginCode)
                if (resp.isSuccess) {
                    qzhqUseCodeData = qzhqUseCodeData?.copy(useCodeStatus = newStatus)
                        ?: com.hualala.linyu.model.QzhqUseCodeData(randomCode = PrefsHelper.qzhqRandomCode, useCodeStatus = newStatus)
    ...
    fun setQzhqUseCode(newCode5: String, onComplete: (Boolean, String) -> Unit) {
        ...
        viewModelScope.launch {
            try {
                val resp = com.hualala.linyu.api.QzhqNetwork.setUseCode(newCode5, phone, loginCode)
                if (resp.isSuccess) {
                    PrefsHelper.qzhqCustomCode = newCode5
                    val codeResp = com.hualala.linyu.api.QzhqNetwork.getUseCode(phone, loginCode)
                    ...
                    qzhqUseCodeData = com.hualala.linyu.model.QzhqUseCodeData(
                        randomCode = random3,
    ...
    fun loginQzhq(phone: String, passwordRaw: String, onComplete: (Boolean, String) -> Unit) {
        viewModelScope.launch {
            try {
                val resp = com.hualala.linyu.api.QzhqNetwork.login(phone, passwordRaw)
                if (resp.isSuccess && resp.data != null) {
                    isQzhqLoggedIn = true
                    loadQzhqUseCode()
                    loadBills()
    ...
    fun smsLoginQzhq(phone: String, smsCode: String, onComplete: (Boolean, String) -> Unit) {
        viewModelScope.launch {
            try {
                val resp = com.hualala.linyu.api.QzhqNetwork.smsLogin(phone, smsCode)
                if (resp.isSuccess && resp.data != null) {
                    isQzhqLoggedIn = true
                    loadQzhqUseCode()
                    loadBills()
    ...
    fun sendQzhqSmsCode(phone: String, onComplete: (Boolean, String) -> Unit) {
        viewModelScope.launch {
            try {
                val resp = com.hualala.linyu.api.QzhqNetwork.sendVerifyCode(phone)
                if (resp.isSuccess) {
                    toastMessage = "验证码已发送"
            ...
            try {
                    ...
                    for ((pid, aid) in targets) {
                        cloudJobs.add(async(Dispatchers.IO) {
                            try {
                                ...
                                com.hualala.linyu.utils.AppLogger.w("BillLoad", "查询项目 $pid ($month) 异常: ${e.message}")
                                emptyList()
                            }
                        })
                    }
                ...
                val qzhqJob = async(Dispatchers.IO) {
                    ...
                    if (qzhqPhone.isNotEmpty() && qzhqLoginCode.isNotEmpty()) {
                        try {
                            val qzhqResp = com.hualala.linyu.api.QzhqNetwork.getWalletBillList(phone = qzhqPhone, loginCode = qzhqLoginCode)
                            if (qzhqResp.isSuccess && !qzhqResp.data.isNullOrEmpty()) {
                                com.hualala.linyu.utils.AppLogger.i("BillLoad", "成功获取后勤洗浴账单共 ${qzhqResp.data.size} 笔")
                                ...
                                latestWithBalance?.afterMoney?.let { bal ->
                                    ...
                                    com.hualala.linyu.utils.AppLogger.i("BillLoad", "已同步最新洗浴结存余额: ¥$balStr")
                                }
                ...
                com.hualala.linyu.utils.AppLogger.i("BillLoad", "成功汇总全区全品类账单共 ${billList.size} 笔 (包含后勤洗浴 ${qzhqBills.size} 笔)")
            } catch (e: Exception) { 
                com.hualala.linyu.utils.AppLogger.e("BillLoad", "加载账单异常: ${e.message}", e)
                checkKickEx(e)
                val msg = e.message ?: ""
                if (msg.contains("Unable to resolve host", ignoreCase = true) ||
                    msg.contains("No address associated", ignoreCase = true) ||
